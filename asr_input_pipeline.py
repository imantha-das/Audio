...
def edit_filenames_from_zoom(folders,  path):
    ...
    """
    if not os.path.exists(os.path.join(path, "zoom-audio")):
        os.mkdir("zoom-audio")
    ...
    for idx, folder in enumerate(folder_ls):
        ...
        if len(listdir_nohidden()) <= 3:          
            for file in listdir_nohidden(): 
                ...
                split_file = re.split('[_.]', file)
                #print(split_file)
                 
                nm = "_".join(split_file[3:-1]) # Get name(s) from file
                if nm != "NTU_Host":
                    # rename file with new name, i.e 2021_10_21-sess01-Gabriel_Yeap-audio
                    fname = sess_date + "-sess0" + str(idx + 1) + "-" + nm + "-audio" + "." + split_file[-1]
        ...
        # There are more than 4 files -- NTU_HOST, Speaker1, NTU_HOST, speaker1 
        elif len(listdir_nohidden()) == 4:
            for file in listdir_nohidden():
                split_file = re.split('[_.]', file)
                # Renameing audio 1
                if len(split_file[2]) > 1: # Searching for 1 from ["audio", "only", "1", ...]
                     nm = "_".join(split_file[3:-1]) # Get name(s) from file
                     if nm != "NTU_Host":
                        fname = sess_date + "-sess0" + str(idx+ 1) + "-" + nm + "-audio" + str(1) + "." + split_file[-1]
                        os.rename(file, fname)
                        print(f"{colored(file, 'green')} --> {colored(fname, 'green')}")
                        # Push file to Downloads/audio-record folder
                        shutil.move(fname, os.path.join(path, "zoom-audio"))
                ...
                # Renameing audio 2
                    ...
                    nm = "_".join(split_file[4:-1])
                    if nm != "NTU_Host":
                        fname = sess_date + "-sess0" + str(idx + 1) + "-" + nm + "-audio" + str(2) +  "." + split_file[-1]
                        os.rename(file, fname)
                        print(f"{colored(file, 'green')} --> {colored(fname, 'green')}")
                        # Push file to Downloads/audio-record folder
                        shutil.move(fname, os.path.join(path, "zoom-audio"))
        else:
            print(colored(f"Multiple session in Audio Record folder", "red", attrs = ["bold"]))

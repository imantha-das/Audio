...
#Description : Scoring pipeline
	...
	# 2. Multi channel srt --> Mono channel textgrid
...
trans_loc=`find ~/Downloads -name "${sess_id}*.srt"`
f_name_srt=`find ~/Downloads -name "${sess_id}*.srt" -exec basename {} \;`
...
trans_srt_store_multi_loc="/home/imantha/speech-and-language-lab/auto-scoring-pipeline/data-for-autoscoring/transcripts/multi-srt-tier"
...
# Create folder with sess_id and store transcript from downloads in folder
if [ -f "$trans_loc" ]
then 
	`mkdir $trans_srt_store_multi_loc"/$sess_id"`
	`mv $trans_loc $trans_srt_store_multi_loc"/$sess_id"`
...
#2. Multi Channel transcription (srt) --> Mono Channel Transcripts
if [ ! -d "$trans_store_mono_loc/$sess_id" ]
then 
	`mkdir $trans_store_mono_loc/$sess_id`
	f_name_only=`cut -d'.' -f 1 <<< $f_name_srt`
	python conversion_scripts/ccsrt2textgrid.py $trans_srt_store_multi_loc"/$sess_id/$f_name_srt" $trans_store_mono_loc"/$sess_id/$f_name_only.TextGrid" True
else
...
then 
	...
	spk1_transcript_file=`find $trans_store_mono_loc"/$sess_id" -name "*_1.TextGrid"`
	spk1_name=`cut -d'-' -f 3 <<< ${spk1_transcript_file##*/}`
	spk1_new_name=$sess_id"-$spk1_name.TextGrid"

	spk2_transcript_file=`find $trans_store_mono_loc"/$sess_id" -name "*_2.TextGrid"`
	spk2_name_with_channel=`cut -d'-' -f 4 <<< ${spk2_transcript_file##*/}`
	spk2_name=`cut -d'_' -f -4 <<< $spk2_name_with_channel`
